...
public class ProductController {
    ...
    public ResponseEntity<?> addProduct(@ModelAttribute ProductRequest productRequest) throws IOException {
        ...
        Category category = objectMapper.readValue(productRequest.getCategory().toString(), Category.class);
    ...
    public ResponseEntity<?> getProducts(
        ...
        List<Product> productNew = productRepository.findProductsByCategory_Name("Product", Sort.by(Sort.Direction.DESC, "id"));
        productNew.stream().filter(p -> p.getCategory().getDeletedAt() == null && p.getDeletedAt() == null).collect(Collectors.toList());

        if ("".equals(cateName)) {
            products = !"asc".equals(sortDir) ? productRepository.findProductsByCategory_Name("Product", Sort.by(Sort.Direction.DESC, sortField)) : productRepository.findProductsByCategory_Name("Product", Sort.by(Sort.Direction.ASC, sortField));
            products = products.stream().filter(p -> p.getCategory().getDeletedAt() == null && p.getDeletedAt() == null).collect(Collectors.toList());
        } else {
            products = !"asc".equals(sortDir) ? productRepository.findProductsByCategory_Name(cateName, Sort.by(Sort.Direction.DESC, sortField)) : productRepository.findProductsByCategory_Name(cateName, Sort.by(Sort.Direction.ASC, sortField));
            products = products.stream().filter(p -> p.getCategory().getDeletedAt() == null && p.getDeletedAt() == null).collect(Collectors.toList());
        }
    ...
    public ResponseEntity<?> getMilkteaProducts(
        ...
        ProductResponse productResponse = new ProductResponse();
        List<Product> products;
        List<Product> productNew = productRepository.findProductsByCategory_NameNotLikeAndCategory_NameNotLike("Snack", "Product", Sort.by(Sort.Direction.DESC, "id"));
        productNew.stream().filter(p -> p.getCategory().getDeletedAt() == null && p.getDeletedAt() == null).collect(Collectors.toList());

        if ("".equals(cateName)) {
            products = !"asc".equals(sortDir) ? productRepository.findProductsByCategory_NameNotLikeAndCategory_NameNotLike("Snack", "Product", Sort.by(Sort.Direction.DESC, sortField)) : productRepository.findProductsByCategory_NameNotLikeAndCategory_NameNotLike("Snack", "Product", Sort.by(Sort.Direction.ASC, sortField));
            products = products.stream().filter(p -> p.getCategory().getDeletedAt() == null && p.getDeletedAt() == null).collect(Collectors.toList());
        } else {
            products = !"asc".equals(sortDir) ? productRepository.findProductsByCategory_Name(cateName, Sort.by(Sort.Direction.DESC, sortField)) : productRepository.findProductsByCategory_Name(cateName, Sort.by(Sort.Direction.ASC, sortField));
            products = products.stream().filter(p -> p.getCategory().getDeletedAt() == null && p.getDeletedAt() == null).collect(Collectors.toList());
